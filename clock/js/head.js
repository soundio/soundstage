(function(window) {
	...
	function CueStream(timer, clock, paramStream, eventStream, toAbsoluteTime) {
		var eventBuffer  = [];
		var paramBuffer  = [];
		var paramBuffers = [];
		...
		var stream = Fn.Stream(function cue() {
	...
	function Head(timer, clock, sequence, transform, target) {
		...
		CueStream(timer, clock, paramStream, eventStream, toAbsoluteTime)
		.each(function(event) {
			target(event, head);
		});
