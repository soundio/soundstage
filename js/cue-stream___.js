...
//console.log('time', time, 'beat', beat || 0, 'startLoc', startLoc);
	...
	function CueSource(notify, stop) {
	...
	assign(CueSource.prototype, {
		...
		start: function start(time, beat) {
			var source    = this;
			...
			function startCue(time) {
				if (time > startTime) {
					cue.t2 = startTime > audio.currentTime ? startTime : audio.currentTime ;
					source.playCue(time);
					return;
				}
